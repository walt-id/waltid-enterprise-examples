export type SimplePidVerificationRequestId =
  | 'france-identitie'
  | 'eudi-reference-wallet'
  | 'german-eudi-wallet';

interface SimplePidRequestMaterial {
  signedRequest?: boolean;
  encryptedResponse?: boolean;
  clientId?: string;
  key?: Record<string, unknown>;
  x5c?: string[];
  verifierInfo?: Array<Record<string, unknown>>;
}

const GERMAN_EUDI_WALLET_KEY = {
  type: 'jwk',
  jwk: {
    kty: 'EC',
    d: 'P_jpnuZYhXY6-A3p96EYXlxsVBgzN2WyrhdPLHiQH94',
    crv: 'P-256',
    kid: 'wtX5fYw8vKVEpcBOMtOHkyCfGLvT_gH2gmEvTTf2UIo',
    x: 'VshNGFJvttuxuPHnG7FAK6FFohlDpm969JOWJb6bt4k',
    y: 'g5jjPMFRbW06lxjcNk2i305gV8iZvFpC-cTh0y9IaeI',
  },
};

const GERMAN_EUDI_WALLET_X5C = [
  '',
];

const GERMAN_VERIFIER_INFO = [
  {
    format: 'jwt',
    data: '..sILGOnOzYfX5VbfjNUQfOgRqziL7Gshq_LSSUJp--kkELqEdETfA2BJ09wJvX2_KJLVFJG3XYlIt_81pu41Rbg',
  },
];

const FRANCE_IDENTITIE_KEY = {
  type: 'jwk',
  jwk: {
    kty: 'EC',
    crv: 'P-256',
    x: '8VoEZqKzypAcaCIQ8hrd2YIRCExkSvJRYiWxi0YNDCY',
    y: 'B19upk_qV_Rzu5y75SJVdUV8aTSUerbDCXAJdkPaZpc',
    d: '1FcMsm7HhM0LK8nRFyDLuy4Ggn1Yg_6yRy5mXjwNV-s',
  },
};

const FRANCE_IDENTITIE_X5C = [
  '',
];

const EUDI_REFERENCE_WALLET_KEY = {
  type: 'jwk',
  jwk: {
    kty: 'EC',
    x: 'pTqq2Z8iAVzn8pwWRpppkR7xybCZesbzEaeTXdNjbCg',
    y: '45GitmCjPgSyGFI0ESOJHHTit5WQZGwEKv0TvM4TV48',
    crv: 'P-256',
    d: 'sD_7b_rtg-FrCB7slY7ptvPv7TwcvjP4XAaM9QbEVRc',
  },
};

const EUDI_REFERENCE_WALLET_X5C = [
  '',
];

const PID_CREDENTIAL_QUERY = {
  credentials: [
    {
      id: 'pid',
      format: 'mso_mdoc',
      meta: {
        doctype_value: 'eu.europa.ec.eudi.pid.1',
      },
      claims: [
        {
          path: ['eu.europa.ec.eudi.pid.1', 'family_name'],
        },
        {
          path: ['eu.europa.ec.eudi.pid.1', 'given_name'],
        },
        {
          path: ['eu.europa.ec.eudi.pid.1', 'birth_date'],
        },
      ],
    },
  ],
};

function createPidRequestBody(material: SimplePidRequestMaterial): Record<string, unknown> {
  const coreFlow: Record<string, unknown> = {
    dcql_query: PID_CREDENTIAL_QUERY,
  };

  if (material.signedRequest !== undefined) coreFlow.signed_request = material.signedRequest;
  if (material.encryptedResponse !== undefined) coreFlow.encrypted_response = material.encryptedResponse;
  if (material.clientId) coreFlow.clientId = material.clientId;
  if (material.key) coreFlow.key = material.key;
  if (material.x5c) coreFlow.x5c = material.x5c;
  if (material.verifierInfo) coreFlow.verifier_info = material.verifierInfo;

  return {
    flow_type: 'cross_device',
    core_flow: coreFlow,
  };
}

const SIMPLE_PID_VERIFICATION_REQUESTS: Record<SimplePidVerificationRequestId, Record<string, unknown>> = {
  'france-identitie': createPidRequestBody({
    signedRequest: true,
    encryptedResponse: true,
    clientId: 'x509_san_dns:verifier2.demo.walt.id',
    key: FRANCE_IDENTITIE_KEY,
    x5c: FRANCE_IDENTITIE_X5C,
  }),
  'eudi-reference-wallet': createPidRequestBody({
    signedRequest: true,
    encryptedResponse: true,
    clientId: 'x509_hash:IEP4DYWPki3n7eibTrkX_zWb9oICgQKDb1Gl3t7cKmk',
    key: EUDI_REFERENCE_WALLET_KEY,
    x5c: EUDI_REFERENCE_WALLET_X5C,
  }),
  'german-eudi-wallet': createPidRequestBody({
    signedRequest: true,
    clientId: 'x509_hash:cbxrATH66Crh_jzd4JUQriuZpBH_vl5Syc-S_JU8fQE',
    key: GERMAN_EUDI_WALLET_KEY,
    x5c: GERMAN_EUDI_WALLET_X5C,
    verifierInfo: GERMAN_VERIFIER_INFO,
  }),
};

function cloneRequestBody(requestBody: Record<string, unknown>): Record<string, unknown> {
  return JSON.parse(JSON.stringify(requestBody)) as Record<string, unknown>;
}

export function getSimplePidVerificationRequestBody(
  id: string,
): Record<string, unknown> | undefined {
  if (id in SIMPLE_PID_VERIFICATION_REQUESTS) {
    return cloneRequestBody(SIMPLE_PID_VERIFICATION_REQUESTS[id as SimplePidVerificationRequestId]);
  }

  return undefined;
}
