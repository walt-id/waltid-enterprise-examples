...
function useQRCode(value: string, size: number) {
...
function QROverlay({
  ...
  if (verified && verificationSuccess) {
    return (
        <div className="absolute inset-0 flex flex-col gap-2 items-center justify-center">
...
function QRCanvas({
  ...
  const { dataUrl, error, isLoading } = useQRCode(value, size);

  return (
      // Fix: container dimensions match the `size` prop, not a hardcoded 200 px
      <div
  ...
  return (
      <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
        ...
        <div className="relative z-10 w-full max-w-md mx-4 my-8 overflow-hidden bg-white rounded-2xl shadow-xl">
          ...
          <div className="p-6">
            <h3 className="text-xl font-medium text-gray-900 mt-5 text-center">{title}</h3>
